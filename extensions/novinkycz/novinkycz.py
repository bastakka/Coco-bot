...
async def _make_new_embed(news) -> discord.Embed:
    ...
    embed.set_footer(
        text=f"Datum zprávy: {news.publication_date}"
    )
    embed.set_image(url=news.preview_image_url)
    return embed
...
class Novinkycz(BaseCog):
    ...
    def _get_news_reposts(self) -> dict:
        ...
        try:
            with open(self.news_repost_json_path, "r", encoding="utf-8") as news_reposts_file:
                news_reposts = json.load(news_reposts_file)
        except FileNotFoundError:
            self.logger.warning("Could not find news_repost.json file, making new one...")
            with open(self.news_repost_json_path, "w", encoding="utf-8") as news_reposts_file:
                news_reposts = {}
    ...
    def _save_news_channels(self) -> None:
        ...
        with open(self.news_channels_json_path, "w", encoding="utf-8") as news_channels_file:
    ...
    def _increment_news(self) -> None:
        ...
        for news_repost in self.news_reposts.copy():
            ...
            if self.news_reposts[news_repost] >= 336:
                self.logger.debug("%s has reached 2 weeks, removing from list", news_repost)
